...
OPTS=`getopt -o ab:Cd:f:h:l:i:p:P:su:vw:x --long audio,adb-path:,device:,flip:,height:,help,adb-flags:,use-wifi:,port:,password:,no-sync,username:,video,width:,no-proxy,no-echo-cancel -n "$0" -- "$@"`
eval set -- "$OPTS"
